...
const NETFLIX_CATALOG = [
  {
    ...
    rating: 7.0,
    summary: "A lone scientist in the Arctic races to stop a team of astronauts from returning home to a mysterious global catastrophe."
  },
  {
    ...
    rating: 9.2,
    summary: "Orphaned chess prodigy Beth Harmon fights addiction and prejudice in her quest to become the world's greatest chess player."
  },
  {
    title: "Squid Game",
    year: 2021,
    ...
    rating: 8.0,
    summary: "Hundreds of cash-strapped contestants accept a strange invitation to compete in children's games for a tempting prize, but the stakes are deadly."
  },
  {
    title: "The Crown",
    year: 2016,
...
const getNetflixContext = (query) => {
    ...
    const generalKeywords = ['recommend', 'rating', 'director', 'genre', 'show me', 'about'];
    const isGeneralCatalogQuery = generalKeywords.some(keyword => lowerQuery.includes(keyword)) ||
                                 NETFLIX_CATALOG.some(item => lowerQuery.includes(item.genre.toLowerCase()));
...
const App = () => {
  ...
  const [chatHistory, setChatHistory] = useState([
    { role: 'model', parts: [{ text: "Hello! I am your personalized Netflix Chatbot. Ask me about our titles, like 'Queen's Gambit' or 'recommend a sci-fi show'." }] }
  ]);
  ...
  const startTypingEffect = (fullText) => {
    ...
    setChatHistory(prev => prev.filter(msg => msg.role !== 'streaming_model'));
    ...
    const intervalId = setInterval(() => {
      if (index < fullText.length) {
        ...
        setChatHistory(prev => [...prev, { role: 'model', parts: [{ text: fullText }] }]);
        setStreamingText('');
      }
    }, TYPING_SPEED);
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 403) {
             throw new Error(`Authentication Error (403): The API Key is likely invalid or missing. Error message: ${errorMessage}`);
          }
        ...
        if (modelResponse) {
          ...
          throw new Error("Model response was empty or malformed. Check console for response structure.");
        }
        ...
        if (i === maxRetries - 1) {
          throw new Error(`Final attempt failed: ${e.message}`);
        }
  ...
  const sendMessage = async () => {
    ...
    setIsLoading(true);
    setError(null);
    const newUserMessage = { role: 'user', parts: [{ text: augmentedUserQuery }] };
      ...
      <div className="w-full max-w-xl bg-white rounded-xl shadow-2xl flex flex-col h-[85vh] transition-all duration-300 border border-gray-100">
        ...
        <header className="p-4 border-b border-blue-100 bg-blue-600 rounded-t-xl shadow-md">
          <h1 className="text-xl font-bold text-white flex items-center">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 inline mr-2" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div ref={chatMessagesRef} className="flex-grow p-4 overflow-y-auto space-y-4 bg-white">
          ...
          {isLoading && !streamingText && (
            <div className="flex justify-start">
              <div className="bg-blue-50 text-blue-700 p-3 rounded-xl rounded-tl-none shadow-md flex items-center space-x-2">
                ...
                <span className="text-sm font-medium">Querying Netflix Catalog...</span>
              </div>
            </div>
          )}
        </div>
        ...
        {error && (
          <div className="p-3 text-center bg-red-100 border-t border-red-300">
            <p className="text-red-700 text-sm font-semibold whitespace-pre-wrap">{error}</p>
          </div>
        )}
        ...
        <div className="p-4 border-t border-gray-200 flex space-x-3 bg-white rounded-b-xl">
          ...
          <button
            ...
            className={`
              ...
              ${(!userInput.trim() || isLoading || !!streamingText) ? 'opacity-50 cursor-not-allowed shadow-none' : ''}
            `}
